...
export default function Amuvino3DBackground() {
...
currentMount.appendChild(renderer.domElement);
        ...
        function createBookSpineTexture(title, bgColor, textColor) {
            ...
            const ambientLight = new THREE.HemisphereLight(0xffe2cc, 0x222235, 0.45);
            scene.add(ambientLight);

            
            const keyLight = new THREE.DirectionalLight(0xffdfb3, 0.35);
            keyLight.position.set(4, 6, 3);
            keyLight.castShadow = true;
            keyLight.shadow.mapSize.set(1024, 1024);
            scene.add(keyLight);

            const fillLight = new THREE.DirectionalLight(0xa6c8ff, 0.2);
            ...
            const deskSpot = new THREE.SpotLight(0xffe0b3, 0.6, 8, Math.PI / 4, 0.5, 1);
            deskSpot.position.set(-0.8, 1.0, 0.6);
            deskSpot.target.position.set(0, -0.1, 0.6);
            deskSpot.castShadow = true;
            scene.add(deskSpot);
            scene.add(deskSpot.target);

            
            const screenGlow = new THREE.PointLight(0x3399ff, 0.15, 1.5);
            screenGlow.position.set(0, -0.1, 0.65);
        ...
        const xOffset = width > 768 ? width * 0.22 : 0;
